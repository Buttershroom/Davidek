...
class GUI:
    ...
    def add_separator(self, row, column):
        tk.Label(self.top,text='_____________________________________').grid(row=row, column=column)
        
    def add_label(self, text, row, column, bold=False):
        """ Adds a label to the GUI when called.
        """
        if bold:
            tk.Label(self.top, text=text, justify=tk.LEFT, font=('bold')).grid(row=row, column=column)
        else:
            tk.Label(self.top, text=text, justify=tk.LEFT).grid(row=row, column=column)
